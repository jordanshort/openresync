...
module.exports = function(mlsSourceName, configBundle, eventEmitter, loggerArg) {
  ...
  if (platformAdapterName === 'bridgeInteractive') {
    ...
    platformAdapter = require('./platformAdapters/utahRealEstate')()
  } else {
    ...
    for (let i = startIndex; i < mlsResources.length; i++) {
      ...
      while (missingIds.length) {
        ...
        const url = (() => {
          ...
          const { reconcileFilterSeparator, reconcileFilterTemplate, getReconcileIdFilterString } = mlsSourceUserConfig;
          const filterTemplate = reconcileFilterTemplate ? reconcileFilterTemplate : `${primaryKey} in (PLACEHOLDER)`
          ...
          while (missingIds.length && recordsToDownloadCount < top) {
            ...
            let maybeSeparator;
            if (reconcileFilterSeparator) {
              maybeSeparator = filterString ? reconcileFilterSeparator : ''
            } else {
              maybeSeparator = filterString ? ',' : ''
            }
            // const idFilterString = maybeSeparator + quote(id)
            const idFilterString = getReconcileIdFilterString ? getReconcileIdFilterString(primaryKey, maybeSeparator, id) :  maybeSeparator + quote(id)
