...
const utahREPlatformDataAdapter = require('../platformDataAdapters/utahRealEstate/cosmos');
...
function buildDataAdapter({ destinationConfig, platformAdapterName }) {
  ...
  if (dataAdapterType === 'mysql') {
    ...
    dataAdapter = cosmosDataAdapter({ destinationConfig: destinationConfig.config })
  } else {
  ...
  if (dataAdapterType === 'mysql') {
    if (platformAdapterName === 'bridgeInteractive') {
      ...
      platformDataAdapter = utahREPlatformDataAdapter();
    } else {
