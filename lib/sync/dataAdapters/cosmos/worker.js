...
const missingOrOldObjs = dataInMls.reduce((a, v, index) => {
  ...
  // console.log('mls data: ', v);
  // console.log('matched dest data: ', dbObj);
  ...
  if (dbObj) {
    for (let i = 0; i < timestampFieldNames.length; i++) {
      ...
      // TO DO something here is breaking the comparison
      const mlsTimestamp = new Date(v[timestampFieldNames[i]])
      let dbTimestamp = dbObj[mysqlTimestampFieldNames[i]]
      dbTimestamp = dbTimestamp.replace(' ', 'T').replace('.000', 'Z');
